...
import { deleteFromTable, insertIntoTable, selectFromTable } from "../config/sql.js";
...
export const createTodoList = tryCatch(async (req, res) => {
  ...
  const { user_id } = req.app.get("user");
    ...
    await insertIntoTable(tableName, ["user_id", "list_name", "time_scheduled"], [user_id, list_name, time_scheduled]);
...
export const getAllTodos = tryCatch(async (req, res) => {
  ...
  const todoList = await selectFromTable(tableName, '*', condition)
...
export const getTodo = tryCatch(async (req, res) => {
  ...
  const nextcondition =  `user_id = ${userId} AND list_id = ${listId}`
  const queriedTodo = await selectFromTable(tableName, '*', nextcondition)
...
export const updateTodo = tryCatch(async (req, res) => {
  ...
  // const getTodoQuery =
  //   "SELECT * FROM todo_lists WHERE user_id = ? AND list_id = ?";
    const nextconditions =  `user_id = ${userId} AND list_id = ${listId}`

  // const todoStatus = await queryPromise(getTodoQuery, [userId, listId]);
  const todoStatus = await selectFromTable(tableName, '*', nextconditions)
...
export const deleteTodo = tryCatch(async (req, res) => {
  ...
  const condition = `task_id = ${list_Id}`
  
  const doesListExist = await selectFromTable(tableName, '*', condition) 
  ...
  const deletecondition = `task_id = ${list_Id}`
  await deleteFromTable(tableName, deletecondition)
